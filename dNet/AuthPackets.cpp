...
void AuthPackets::HandleLoginRequest(dServer* server, Packet* packet) {
	...
	stamps.push_back(Stamp(eStamps::START, 0, time(nullptr)));
	...
	stamps.push_back(Stamp(eStamps::CLIENT_OS, 0, time(nullptr)));
	...
	if (!accountInfo) {
		...
		stamps.push_back(Stamp(eStamps::ERROR, 1, time(nullptr)));
	...
	if (Game::config->GetValue("dont_use_keys") != "1" && accountInfo->maxGmLevel == eGameMasterLevel::CIVILIAN) {
		...
		stamps.push_back(Stamp(eStamps::BYPASS, 1, time(nullptr)));
	}
	...
	if (!loginSuccess) {
		...
		if (!server->GetIsConnectedToMaster()) {
			stamps.push_back(Stamp(eStamps::WORLD_DISCONNECT, 1, time(nullptr)));
			AuthPackets::SendLoginResponse(server, system, eLoginResponse::GENERAL_FAILED, "", "", 0, username, stamps);
			return;
		}
		stamps.push_back(Stamp(eStamps::WORLD_SESSION_CONFIRM_TO_AUTH, 1, time(nullptr)));
...
void AuthPackets::SendLoginResponse(dServer* server, const SystemAddress& sysAddr, eLoginResponse responseCode, const std::string& errorMsg, const std::string& wServerIP, uint16_t wServerPort, std::string username, std::vector<Stamp>& stamps) {
	stamps.push_back(Stamp(eStamps::IM_LOGIN_START, 1, time(nullptr)));
	...
	stamps.push_back(Stamp(eStamps::WORLD_COMMUNICATION_FINISH, 1, time(nullptr)));
