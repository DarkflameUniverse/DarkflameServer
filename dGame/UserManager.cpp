...
void UserManager::DeleteCharacter(const SystemAddress& sysAddr, Packet* packet) {
	...
	if (!hasCharacter) {
		Log("User %i tried to delete a character that it does not own!", u->GetAccountID());
...
void UserManager::RenameCharacter(const SystemAddress& sysAddr, Packet* packet) {
	...
	if (!hasCharacter || !character) {
		Log("User %i tried to rename a character that it does not own!", u->GetAccountID());
		...
		LogError("Could not rename character, either hasCharacter or character variable != true.");
		WorldPackets::SendCharacterRenameResponse(sysAddr, eRenameResponse::UNKNOWN_ERROR);
	}
}
...
void UserManager::LoginCharacter(const SystemAddress& sysAddr, uint32_t playerID) {
	...
	if (hasCharacter && character) {
		...
		LogError("Could not login character, either hasCharacter %i or character variable != true %i.", hasCharacter, character != nullptr);
	}
}
