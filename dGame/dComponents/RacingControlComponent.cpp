			...
			if (respawnIndex == 0) {
				...
				if (missionComponent != nullptr) {
					...
					if (player.lap == 3) {
						...
						Log("Completed time %llu, %llu", raceTime, raceTime * 1000);
				...
				Log("Lapped (%i) in (%llu)", player.lap, lapTime);
			}

			Log("Reached point (%i)/(%i)", player.respawnIndex, path->pathWaypoints.size());
