...
void ControllablePhysicsComponent::RemovePickupRadiusScale(float value) {
	...
	if (pos != m_ActivePickupRadiusScales.end()) {
		...
		LogWarning("Could not find pickup radius %f in list of active radii.  List has %i active radii.", value, m_ActivePickupRadiusScales.size());
		return;
	}
...
void ControllablePhysicsComponent::RemoveSpeedboost(float value) {
	...
	if (pos != m_ActiveSpeedBoosts.end()) {
		...
		LogWarning("Could not find speedboost %f in list of active speedboosts.  List has %i active speedboosts.", value, m_ActiveSpeedBoosts.size());
		return;
	}
