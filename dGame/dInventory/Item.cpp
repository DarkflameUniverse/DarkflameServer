...
bool Item::Consume() {
	...
	LogDebug("Consumed LOT (%i) itemID (%llu).  Success=(%d)", lot, id, success);
...
void Item::UseNonEquip(Item* item) {
	...
	if (!GetInventory()) {
		LogDebug("item %i has no inventory??", this->GetLot());
		return;
	}

	auto* playerInventoryComponent = GetInventory()->GetComponent();
	if (!playerInventoryComponent) {
		LogDebug("no inventory component attached to item id %llu lot %i", this->GetId(), this->GetLot());
		return;
	}

	auto* playerEntity = playerInventoryComponent->GetParent();
	if (!playerEntity) {
		LogDebug("no player entity attached to inventory? item id is %llu", this->GetId());
		return;
	}
	...
	if (type == eItemType::MOUNT) {
		...
		LogDebug("Player %llu %s used item %i", playerEntity->GetObjectID(), success ? "successfully" : "unsuccessfully", thisLot);
